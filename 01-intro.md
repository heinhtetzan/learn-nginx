...
At the time, most web servers (like **Apache**) used a *process-per-connection* model, which caused heavy CPU/memory usage. Nginx introduced a new model: **asynchronous, event-driven architecture**, which could handle many connections with minimal resources.
...
| **Modular and configurable** | Flexible for many architectures (microservices, APIs, etc.). |
