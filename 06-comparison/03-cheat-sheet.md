...
| **Server Tokens** | `ServerTokens Prod` | `server_tokens off;` | `header -Server` | `server_tokens false` |

---

## 5. Operational CLI Commands Comparison

| Action | Apache | NGINX | Caddy | Ferron |
| :--- | :--- | :--- | :--- | :--- |
| **Syntax Test** | `apachectl configtest` | `nginx -t` | `caddy validate --config Caddyfile` | `ferron --config ferron.kdl --validate` |
| **Graceful Reload** | `apachectl -k graceful` | `nginx -s reload` | `caddy reload` | `kill -HUP $(pgrep ferron)` |
| **Check Version** | `apachectl -V` | `nginx -V` | `caddy version` | `ferron --version` |
